...
export const useConfirmationInmediate = () => {
  ...
  const {
    values: {concept, termsAreAccepted},
    form,
  } = confirmationInmediate;
  ...
  const isBtnConfirmationInmediateDisabled =
    !termsAreAccepted || hasFavorites
      ? !isValidAccountFav || hasErrors
      : hasErrors;

  const handleSubmitConfirmationInmediate = async () => {
    ...
    const payload: OtherBanksProps = {
      ...
      concept: concept,
      transactionType: 'I',
      mpe001IDL: queryInfo.mpe001IDL,
      transferId: queryInfo.transferId,
    };
